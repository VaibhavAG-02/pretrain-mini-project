...
Uses langdetect to identify language and filters for English.
"""
...
def detect_language(text: str) -> tuple:
    ...
    Returns:
        tuple: (language_code, confidence, is_reliable)
    """
    try:
        # Use first 1000 chars for detection
        sample = text[:1000]
        lang = detect(sample)
        # langdetect doesn't provide probability, so we return 1.0 for successful detection
        return lang, 1.0, True
...
def process_file(
    ...
    Args:
        ...
        language_filter: Language to keep (default 'en')
    ...
    for row in tqdm(df.iter_rows(named=True), total=len(df)):
        ...
        if row.get('source') == 'code':
            ...
            lang, prob, reliable = detect_language(normalized)
    ...
    print(f"\n   Filtering reasons:")
    print(f"      Wrong language: {len(df.filter((pl.col('language_detected') != language_filter) & (pl.col('source') != 'code')))}")
    print(f"      Invalid length: {len(df.filter(~pl.col('valid_length')))}")
    print(f"      Insufficient words: {len(df.filter(~pl.col('sufficient_words')))}")
